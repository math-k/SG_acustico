...
do k=1, passos
    ...
    if (nstreamer_head < nstreamer_length) then
        do t=1,nstreamer_head,nspacing
            seismogram(k,t)=P(nreceptor_z,t)
        end do
    else
        do t=nstreamer_tail,nstreamer_head,nspacing
        seismogram(k,t)=P(nreceptor_z,t)
        end do
    end if
...
nfx=nint(fx/h)+1
nfz=nint(fz/h)+1
nreceptor_z=nint(receptor_z/h)+1
nstreamer_tail=nint(streamer_tail/h)
nstreamer_head=nint(streamer_head/h)+1
...
c_image=0
vetor_cerjan=0
nstreamer_length=nstreamer_head-nstreamer_tail+1
